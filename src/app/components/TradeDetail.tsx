...
export default function TradeDetail() {
  ...
  const fetchTradeDetail = async () => {
    setLoading(true);
    try {
      // 1. 게시글 상세 정보 (Home.tsx와 동일하게 title, content 사용)
      ...
      // 2. 찜 여부 확인 (recipe_id 컬럼명이 맞는지 확인 필요)
      const {
        data: { user },
      } = await supabase.auth.getUser();
      if (user) {
  ...
  return (
    <div className="min-h-screen bg-white pb-32">
      ...
      <div className="p-4">
        ...
        {/* Home.tsx와 필드명 통일: item_name -> title */}
        <h1 className="text-xl font-bold mb-2">{trade.title}</h1>
