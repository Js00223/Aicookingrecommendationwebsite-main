...
/* Table schema:
CREATE TABLE kv_store_8f042a09 (
...
export const del = async (key: string): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_8f042a09").delete().eq("key", key);
  if (error) {
    throw new Error(error.message);
  }
};
...
export const mdel = async (keys: string[]): Promise<void> => {
  const supabase = client()
  const { error } = await supabase.from("kv_store_8f042a09").delete().in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
};
